...
public class AStockTradeUtility implements ITradeUtility {
    ...
    public Boolean isTradeTime() {
        ...
        String shortNowDate = now.toLocalDate().toString();

        if(Arrays.asList(tradeDays_2017_year).contains(shortNowDate)) {

            LocalDateTime amStart = LocalDateTime.parse(String.format("%s 09:30", shortNowDate), formatterWithTime);
            LocalDateTime amEnd = LocalDateTime.parse(String.format("%s 11:30", shortNowDate), formatterWithTime);
            LocalDateTime pmStart = LocalDateTime.parse(String.format("%s 13:00", shortNowDate), formatterWithTime);
            LocalDateTime pmEnd = LocalDateTime.parse(String.format("%s 15:00", shortNowDate),formatterWithTime);
            if((now.compareTo(amStart) >= 0 && now.compareTo(amEnd) <= 0) || (now.compareTo(pmStart) >= 0 && now.compareTo(pmEnd) <= 0)) {
                return true;
            }
        }
